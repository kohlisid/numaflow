...
type FlatmapApplier interface {
	...
	ApplyMap(ctx context.Context, messageStream []*isb.ReadMessage, responseCh chan<- *types.ReadWriteMessagePair) (chan struct{}, <-chan error)
}

// ApplyFlatmapFunc utility function used to create a FlatmapApplier implementation
type ApplyFlatmapFunc func(ctx context.Context, messageStream []*isb.ReadMessage, responseCh chan<- *types.ReadWriteMessagePair) (chan struct{}, <-chan error)

func (f ApplyFlatmapFunc) ApplyMap(ctx context.Context, messageStream []*isb.ReadMessage, responseCh chan<- *types.ReadWriteMessagePair) (chan struct{}, <-chan error) {
	return f(ctx, messageStream, responseCh)
}
